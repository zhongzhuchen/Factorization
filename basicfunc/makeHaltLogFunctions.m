function [halt_log_fn, get_log_fn] = makeHaltLogFunctions(maxit,k,s)
%   makeHaltLogFunctions: (examples/ex3)
...
%
%   examples/ex3/makeHaltLogFunctions.m introduced in GRANSO Version 1.5.
%
% =========================================================================
...
% =========================================================================
    ...
    f           = zeros(1,maxit+1);
    tv          = zeros(1,maxit+1);
    steplength    = zeros(1,maxit+1);
    
    % indicator of whether the current iterates is differentiable: if
    % (lambda_{n-s} Theta-lambda_{n-s+1} Theta)/lambda_{n-s} Theta>1e-6, we can say the
    % objective function is differentiable at current iterates and the
    % indicator is 1; otherwise we say that the objective function is
    % approximately non-differentible at current iterate and the indicator
    % is 0
    diff_ind=zeros(1,maxit+1);
    ...
    function log = getLog()
        ...
        % return history of objective values, total violations, step
        % lengths and differentiability indication
        log.x   = x_iterates(1:index);
        log.f   = f(1:index);
        log.tv  = tv(1:index);
        log.steplength=steplength(1:index);
        log.diff_ind=diff_ind(1:index);
    end
end