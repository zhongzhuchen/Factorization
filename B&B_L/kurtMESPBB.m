function [x,obj,info] = kurtMESPBB(s,C,ubfunc)
% Kurt's branch and bound code for solving MESP problems
%{
C       - data matrix
s       - size of subset to choose
ubfunc  - upper bound function to use
...
if prod(lam)> 0
    ...
    Cinv=U*diag(1./lam)*U';       % HEURSITIC ON ORIGINAL
...
gamma=Linx_gamma(C,s);      % initial gamma value
...
%         tStart=cputime;
    ...
    if complement                   % apply bound to complement of node problem
        [results,xval,delta_one,delta_zero]=ubfunc(Cnodeinv,nnode-snode,control); % note deltas switched  
        nodeconstant=fixconstant+logdetCnode;
        xval=ones(nnode,1)-xval;
    else 
        [results,xval,delta_zero,delta_one]=ubfunc(Cnode,snode,control);           % use original problem
