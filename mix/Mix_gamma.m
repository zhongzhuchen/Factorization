function [optgamma,optbound] = Mix_gamma(C,s,alpha,mix_handle)
% Interior point method to optimize scale factor for Linx bound under mixing setting
% the mix_handle is created to include all necessary parameters itself
% alpha is the mixing parameter associated with the Linx bound
[n,~]=size(C);
...
c1=1e-4;
c2=0.9;
%solve the mixing bound for gamma and obtain x
[x,bound,~]=mix_handle(C,s,gamma);
AUX = C*diag(x)*C;
%Compute F(gamma,x)
...
gap=bound-heurval;
res= alpha*(n - s - trace(Finv*diag(ones(n,1)-x))); 
allgamma=gamma;
allres=res;
allbound=bound;
...
while(k<=Numiterations && gap > TOL && abs(res) > TOL && difgap > TOL) 
    ...
    Mat = alpha*gamma*(ones(n,1)-x)'*diag(Finv*AUX*Finv);
    ...
    [nx,nbound,~]=mix_handle(C,s,ngamma);
    nAUX = C*diag(nx)*C;
    ...
    nres= alpha*(n - s - trace(nFinv*diag(ones(n,1)-nx))); 
